# The modularization handbook - Epoch style, v0.2
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_epoch/modules/shuttle_toggle`
...
We will no longer have _epoch map versions.
...
- ***Example:*** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_epoch/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_epoch/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_epoch/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_epoch/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_epoch/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_epoch/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes Epoch edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. Epoch edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  //EPOCH EDIT ADDITION BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  var/adminEmergencyNoRecall = FALSE
  var/lastMode = SHUTTLE_IDLE
  var/lastCallTime = 6000
  //EPOCH EDIT ADDITION END
  ```

- **Removal:**

  ```byond
  //EPOCH EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  /*
  for(var/obj/docking_port/stationary/S in stationary)
    if(S.id = id)
      return S
  */
  //EPOCH EDIT REMOVAL END
  ...
  ```byond
  //EPOCH EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Moved to modular_epoch/shuttle_toggle/randomverbs.dm)
  /*
...
- **Change:**
  ...
  ```byond
  //EPOCH EDIT CHANGE BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  //if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE) - EPOCH EDIT - ORIGINAL
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED)
  //EPOCH EDIT CHANGE END
      return 1
  ```
...
That folder is **`code/__DEFINES/~epoch_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_epoch/modules/example_module/code/disease_mob.dm
- Bad: /modular_epoch/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_epoch/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_epoch/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for Epoch UIs.
...
You can do both `// EPOCH EDIT` and `/* EPOCH EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// EPOCH EDIT ADDITION START - another example, multiline changes
	icon="rat"
...
```js
// THIS IS A EPOCH UI FILE
```

This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a Epoch edit comment in a modular TGUI file.
